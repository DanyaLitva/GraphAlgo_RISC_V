...
// MCA parallel vectorized (generic)
template<typename T, typename U>
void _mspgemm_mca_parallel_vectorized_m1(const sparseMtx<T> &A, const sparseMtx<T> &B, const sparseMtx<U> &M, sparseMtx<T> &C) {
#ifdef USE_RVV
  int mca_len = 0;
  for (size_t i = 0; i < A.m; ++i)
    if (M.Rst[i + 1] - M.Rst[i] > mca_len)
...
// MCA parallel vectorized (generic)
template<typename T, typename U>
void _mspgemm_mca_parallel_vectorized_m2(const sparseMtx<T> &A, const sparseMtx<T> &B, const sparseMtx<U> &M, sparseMtx<T> &C) {
#ifdef USE_RVV
  int mca_len = 0;
  for (size_t i = 0; i < A.m; ++i)
    if (M.Rst[i + 1] - M.Rst[i] > mca_len)
...
// MCA parallel vectorized (generic)
template<typename T, typename U>
void _mspgemm_mca_parallel_vectorized_m4(const sparseMtx<T> &A, const sparseMtx<T> &B, const sparseMtx<U> &M, sparseMtx<T> &C) {
#ifdef USE_RVV
  int mca_len = 0;
  for (size_t i = 0; i < A.m; ++i)
    if (M.Rst[i + 1] - M.Rst[i] > mca_len)
...
extern int rvv_test_lmul;
// MCA dispatchers
template<typename T, typename U>
sparseMtx<T> mspgemm_mca_m(bool isParallel, bool isVectorization, const sparseMtx<T> &A, const sparseMtx<U> &B, const sparseMtx<T> &M) {
    sparseMtx<T> C(A.m, B.n, M.nz);
    memcpy(C.Col, M.Col, M.nz * sizeof(int));
    memcpy(C.Rst, M.Rst, (M.m + 1) * sizeof(int));

    if (rvv_test_lmul == 1) _mspgemm_mca_parallel_vectorized_m1(A, B, M, C);
    else if(rvv_test_lmul == 4) _mspgemm_mca_parallel_vectorized_m4(A, B, M, C);
    else _mspgemm_mca_parallel_vectorized_m2(A, B, M, C);

    return C;
}

template<typename T, typename U>
void mspgemm_mca_m(bool isParallel, bool isVectorization, const sparseMtx<T> &A, const sparseMtx<T> &B, const sparseMtx<U> &M, sparseMtx<T> &C) {
    C.resize_rows(M.m);
    C.resize_vals(M.nz);
    C.n = M.n;
    memcpy(C.Col, M.Col, C.nz * sizeof(int));
